  ...
  class GAVNodeSet
    ...
    def get_longest_values
      @wanted_fields.each do |f|
        # Because our methods are dynamically built, it's possible to call (say)
        # 'scope()' on an instance that doesn't have a scope line. We'll trap that
        # exception here.
        begin
          @longest_val_table[f.to_sym] = 
            @nodes.map{|n| n.send(f.to_sym).length}.max        
        rescue NoMethodError => e
          STDERR.puts("#{self.class} squelched error " + e) if @DEBUG
        end
    ...
    def prepare_nodeset
      ...
      @nodes.each do |n|
        ...
        @wanted_fields.each do |f|
          begin
            longest_length = @longest_val_table[f.to_sym].to_i
            current_length = n.send(f.to_sym).length      
            spacer =  longest_length - current_length
